...
export default function LandingPage() {
  ...
  const tc = useTranslations('landing.curriculum')
  const tt = useTranslations('landing.testimonials')
  const ta = useTranslations('landing.cta')
  const fo = useTranslations('landing.footer')
  ...
  const NAV_LINKS = [
    ...
    ['#modules',      tn('modules')      ],
    ['#testimonials', tn('testimonials') ],
    ['/blog',         tn('blog')         ],
  ]

  const FEATURES = [
    { icon: '🎯', title: tf('items.bkt.title'),           desc: tf('items.bkt.desc'),           color: C.violet,   bg: C.violetLt  },
    { icon: '✍️', title: tf('items.sheets.title'),        desc: tf('items.sheets.desc'),        color: C.orange,   bg: C.orangeLt  },
    { icon: '🎵', title: tf('items.audio.title'),         desc: tf('items.audio.desc'),         color: C.green,    bg: C.greenLt   },
    { icon: '🏆', title: tf('items.gamification.title'),  desc: tf('items.gamification.desc'),  color: C.blue,     bg: C.blueLt    },
    { icon: '📱', title: tf('items.multiplatform.title'), desc: tf('items.multiplatform.desc'), color: '#9C27B0',  bg: '#F3E5F5'   },
    { icon: '🌍', title: tf('items.languages.title'),     desc: tf('items.languages.desc'),     color: '#E91E63',  bg: '#FCE4EC'   },
  ]

  const MODULES = [
    { num: 1, title: 'Maktab — مَكْتَبٌ', letters: ['م','ك','ت','ب'], desc: tc('module1desc'), color: C.violet, bg: C.violetLt, available: true  },
    { num: 2, title: 'Module 2',           letters: ['ا','و','ي'],     desc: tc('module2desc'), color: C.orange, bg: C.orangeLt, available: false },
    { num: 3, title: 'Module 3',           letters: ['س','ن','ل'],     desc: tc('module3desc'), color: C.green,  bg: C.greenLt,  available: false },
  ]
  ...
  const STATS = [
    { value: '1000', label: ts('lessons')   },
    { value: '6000', label: ts('exercises') },
    { value: '36',   label: ts('modules')   },
    { value: '6',    label: ts('degrees')   },
  ]
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: "'Segoe UI', system-ui, sans-serif", overflowX: 'hidden' }}>
      ...
      {/* ── NAVBAR ── */}
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, background: navBg ? 'rgba(255,255,255,0.97)' : 'transparent', backdropFilter: navBg ? 'blur(12px)' : 'none', borderBottom: navBg ? `1px solid ${C.border}` : 'none', transition: 'all .3s' }}>
        <div style={{ padding: '0 20px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 24 }} className="desktop-only">
            {NAV_LINKS.map(([href, label]) => (
              <a key={href} href={href} style={{ fontSize: 14, fontWeight: 600, color: subColor, textDecoration: 'none', transition: 'color .2s' }}>{label}</a>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }} className="desktop-only">
              ...
              <Link href="/login" style={{ padding: '8px 16px', borderRadius: 10, border: `2px solid ${navBg ? C.border : 'rgba(255,255,255,0.4)'}`, background: 'transparent', color: textColor, fontSize: 13, fontWeight: 600, textDecoration: 'none', transition: 'all .2s' }}>
                {tn('login')}
              </Link>
      ...
      {/* ── HERO ── */}
      <div style={{ background: `linear-gradient(135deg, ${C.violetDk} 0%, ${C.violet} 50%, #9B59B6 100%)`, minHeight: '100vh', display: 'flex', alignItems: 'center', padding: '100px 20px 60px', position: 'relative', overflow: 'hidden' }}>
        {ARABIC_LETTERS.map((letter, i) => (
          <div key={i} style={{ position: 'absolute', left: `${5 + (i * 7) % 90}%`, top: `${10 + (i * 13) % 80}%`, fontFamily: "'Noto Naskh Arabic', serif", fontSize: `${24 + (i % 4) * 16}px`, color: `rgba(255,255,255,${i === activeLetterIdx ? 0.15 : 0.05})`, direction: 'rtl', transition: 'opacity .5s', pointerEvents: 'none', userSelect: 'none' }}>{letter}</div>
        ))}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 48, alignItems: 'center' }}>
            <div>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.12)', padding: '6px 14px', borderRadius: 20, marginBottom: 20 }}>
                <span>🌙</span>
                <span style={{ fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,0.9)' }}>{th('badge')}</span>
              </div>
              <h1 style={{ fontSize: 'clamp(30px, 5vw, 52px)', fontWeight: 800, color: '#fff', lineHeight: 1.1, marginBottom: 16, letterSpacing: '-.02em' }}>
                {th('title1')}<br /><span style={{ color: C.orange }}>{th('title2')}</span>
              </h1>
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <div style={{ background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(20px)', borderRadius: 24, padding: 24, border: '2px solid rgba(255,255,255,0.15)', width: '100%', maxWidth: 360 }}>
                <div style={{ background: 'rgba(255,255,255,0.08)', borderRadius: 14, padding: 16, textAlign: 'center', marginBottom: 14 }}>
                  <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)', marginBottom: 10 }}>{th('demo.question')}</p>
                ...
                <div style={{ textAlign: 'center', padding: '10px', background: 'rgba(240,124,30,0.2)', borderRadius: 10, marginTop: 4 }}>
                  <span style={{ fontSize: 13, fontWeight: 700, color: C.orange }}>{th('demo.excellent')}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <section id="features" style={{ padding: '60px 20px', maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 48 }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: C.violet, background: C.violetLt, padding: '4px 14px', borderRadius: 20, display: 'inline-block', marginBottom: 14 }}>{tf('badge')}</span>
          <h2 style={{ fontSize: 'clamp(22px, 4vw, 36px)', fontWeight: 800, color: C.text, marginBottom: 12 }}>{tf('title')}</h2>
          <p style={{ fontSize: 15, color: C.text2, maxWidth: 500, margin: '0 auto' }}>{tf('desc')}</p>
        </div>
      ...
      <section id="modules" style={{ padding: '60px 20px', background: C.violetLt }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: C.violet, background: C.white, padding: '4px 14px', borderRadius: 20, display: 'inline-block', marginBottom: 14 }}>{tc('badge')}</span>
            <h2 style={{ fontSize: 'clamp(22px, 4vw, 36px)', fontWeight: 800, color: C.text, marginBottom: 12 }}>{tc('title')}</h2>
            <p style={{ fontSize: 15, color: C.text2, maxWidth: 460, margin: '0 auto' }}>{tc('desc')}</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: 14 }}>
            {MODULES.map((mod, i) => (
              <div key={i} style={{ background: C.white, border: `2px solid ${mod.available ? mod.color+'40' : C.border}`, borderRadius: 20, padding: '22px', position: 'relative', opacity: mod.available ? 1 : 0.65, transition: 'all .2s' }}
                ...
                <span style={{ position: 'absolute', top: 12, right: 12, fontSize: 10, fontWeight: 700, background: mod.available ? mod.bg : '#F0F0F0', color: mod.available ? mod.color : C.text3, padding: '2px 8px', borderRadius: 8 }}>
                  {mod.available ? tc('available') : tc('comingSoon')}
                </span>
                ...
                {mod.available
                  ? <Link href="/register" style={{ display: 'block', padding: '10px', borderRadius: 12, background: mod.color, color: '#fff', fontSize: 14, fontWeight: 700, textDecoration: 'none', textAlign: 'center' as const }}>{tc('startBtn')}</Link>
                  : <div style={{ padding: '10px', borderRadius: 12, background: '#F0F0F0', color: C.text3, fontSize: 13, fontWeight: 600, textAlign: 'center' as const }}>{tc('preparingBtn')}</div>
                }
              </div>
            ))}
          </div>
      ...
      <section id="testimonials" style={{ padding: '60px 20px', maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 48 }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: C.orange, background: C.orangeLt, padding: '4px 14px', borderRadius: 20, display: 'inline-block', marginBottom: 14 }}>{tt('badge')}</span>
          <h2 style={{ fontSize: 'clamp(22px, 4vw, 36px)', fontWeight: 800, color: C.text, marginBottom: 12 }}>{tt('title')}</h2>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: 14 }}>
          {TESTIMONIALS.map((item, i) => (
            <div key={i} style={{ background: C.white, border: `2px solid ${C.border}`, borderRadius: 18, padding: '20px' }}>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 12 }}>
                <div style={{ width: 40, height: 40, borderRadius: '50%', background: C.violetLt, color: C.violet, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 700, flexShrink: 0 }}>{item.name[0]}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: 14, fontWeight: 700, color: C.text }}>{item.name}</p>
                  <p style={{ fontSize: 12, color: C.text3 }}>{item.country}</p>
                </div>
                <div style={{ background: C.orangeLt, color: C.orange, padding: '3px 8px', borderRadius: 10, fontSize: 11, fontWeight: 700, flexShrink: 0 }}>{item.xp} XP</div>
              </div>
              <p style={{ fontSize: 13, color: C.text2, lineHeight: 1.7, fontStyle: 'italic' }}>"{item.text}"</p>
      ...
      <section style={{ padding: '60px 20px' }}>
        <div style={{ maxWidth: 720, margin: '0 auto', background: `linear-gradient(135deg, ${C.violetDk}, ${C.violet})`, borderRadius: 28, padding: '48px 28px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <h2 style={{ fontSize: 'clamp(20px, 4vw, 32px)', fontWeight: 800, color: '#fff', marginBottom: 12 }}>{ta('title')}</h2>
          <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.8)', marginBottom: 24, maxWidth: 440, margin: '0 auto 24px' }}>{ta('desc')}</p>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link href="/register" style={{ padding: '13px 28px', borderRadius: 14, background: C.orange, color: '#fff', fontSize: 15, fontWeight: 700, textDecoration: 'none' }}>{ta('registerBtn')}</Link>
            <Link href="/login" style={{ padding: '13px 22px', borderRadius: 14, background: 'rgba(255,255,255,0.12)', color: '#fff', fontSize: 14, fontWeight: 600, textDecoration: 'none', border: '2px solid rgba(255,255,255,0.2)' }}>{ta('loginBtn')}</Link>
          </div>
        </div>
      </section>
      ...
      <footer style={{ background: C.text, padding: '28px 20px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 14 }}>
          ...
          <p style={{ fontSize: 12, color: '#6A6A8A' }}>{fo('copyright')}</p>
          <div style={{ display: 'flex', gap: 16 }}>
            {[fo('privacy'), fo('terms'), fo('contact')].map(link => (
