...
const ACTION_COLORS: Record<string, { color: string; bg: string }> = {
  ...
  'settings.update':     { color: C.pink,   bg: C.pinkLt   },
}
...
export default function SuperAdminAuditPage() {
  ...
  useEffect(() => {
    api.get('/api/v1/admin/admins').then(r => setAdmins(r.data)).catch(() => {})
    fetchLog()
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ background: C.white, border: `2px solid ${C.border}`, borderRadius: 20, overflow: 'hidden' }}>
        ...
        {/* Header tableau */}
        <div style={{ display: 'grid', gridTemplateColumns: '160px 120px 120px 100px 80px 80px 30px', padding: '11px 20px', background: C.bg, borderBottom: `2px solid ${C.border}` }}>
        ...
        {loading ? (
          ...
          return (
            <div key={entry.id}
              style={{ display: 'grid', gridTemplateColumns: '160px 120px 120px 100px 80px 80px 30px', padding: '11px 20px', borderBottom: i < filtered.length - 1 ? `1px solid ${C.border}` : 'none', alignItems: 'center', cursor: 'pointer', transition: 'background .1s' }}
              onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = C.bg}
              onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = C.white}
              onClick={() => setSelected(entry)}>

              <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 8px', borderRadius: 8, background: cfg.bg, color: cfg.color, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'inline-block', maxWidth: 150 }}>
                {entry.action}
              </span>
              ...
              <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 8, background: entry.status === 'success' ? C.greenLt : C.redLt, color: entry.status === 'success' ? C.green : C.red, display: 'inline-block' }}>
                {entry.status}
              </span>
          ...
          <div style={{ background: C.white, borderRadius: 20, padding: '28px', width: '100%', maxWidth: 500 }}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              {[
                ...
                { label: 'Ressource', value: selected.resource_type ? `${selected.resource_type}${selected.resource_id ? ' #' + selected.resource_id : ''}` : '—' },
                ...
                <div key={row.label} style={{ display: 'flex', gap: 12, padding: '10px 0', borderBottom: `1px solid ${C.border}` }}>
              ...
              {Object.keys(selected.details || {}).length > 0 && (
                <div style={{ marginTop: 12 }}>
                  ...
                  <pre style={{ background: C.bg, borderRadius: 10, padding: '12px', fontSize: 11, color: C.text, overflowX: 'auto', margin: 0 }}>
