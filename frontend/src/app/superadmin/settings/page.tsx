...
export default function SuperAdminSettingsPage() {
  ...
  const [settings, setSettings] = useState({
    ...
    notify_payment: true,
    alert_email: 'superadmin@arabiq.com',
    // Backup
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      // await api.post('/api/v1/superadmin/settings', settings)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 760 }}>
      ...
      <Section title="Sécurité" icon="🔐">
        ...
        <Row label="Expiration de session" desc="Durée avant déconnexion automatique (heures)">
          <input type="number" value={settings.session_timeout_hours} onChange={e => update('session_timeout_hours', Number(e.target.value))}
        ...
        <Row label="Tentatives de connexion max" desc="Nombre d'échecs avant blocage du compte">
          <input type="number" value={settings.max_failed_logins} onChange={e => update('max_failed_logins', Number(e.target.value))}
      ...
      <Section title="Notifications" icon="🔔">
        <Row label="Email d'alerte" desc="Reçoit toutes les notifications critiques">
          <input type="email" value={settings.alert_email} onChange={e => update('alert_email', e.target.value)}
      ...
      <Section title="Sauvegarde automatique" icon="💾">
        ...
        <Row label="Fréquence de sauvegarde" desc="Nombre de jours entre chaque sauvegarde">
          <input type="number" value={settings.backup_frequency_days} onChange={e => update('backup_frequency_days', Number(e.target.value))}
        ...
        <Row label="Dernière sauvegarde" desc="">
          <span style={{ fontSize: 12, color: C.green, fontWeight: 600 }}>✓ Il y a 2 jours</span>
        </Row>
      ...
      <div style={{ background: C.redLt, border: `2px solid ${C.red}30`, borderRadius: 20, padding: '24px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          {[
            { label: '🗑 Vider le cache Redis',         action: 'cache'   },
